...
def plot_covid(
    ...
    # 🎯 Dynamisch gecentreerde bars voor correcte uitlijning van jaartallen
    active_metrics = []
    if total_reported:
        active_metrics.append(('Total_reported', 'steelblue', 'Reported'))
    if hospital_admission:
        active_metrics.append(('Hospital_admission', 'seagreen', 'Hospital'))
    if deceased:
        active_metrics.append(('Deceased', 'darkorange', 'Deceased'))
    ...
    for i, (col, color, label) in enumerate(active_metrics):
        offset = (i - (num_metrics - 1) / 2) * bar_width
        ax.bar(x + offset, grouped_df[col], width=bar_width, color=color, label=label)
    ax.set_xticks(x)
    ...
    ax.set_title(f'COVID-19 data for {year} ({province})', fontsize=10, fontweight='normal')
